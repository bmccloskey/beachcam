...
err <- try(manifest_file <- tail(strsplit(grep(paste0("status_", date, "_", hour_s), strsplit(getURL(remote_dir), "\r*\n")[[1]], value = T), " ")[[1]], n = 1))
if (inherits(err, "try-error")) {
  report <- paste("No manifest file found for", hour)
} else {
  hour_s <- substr(manifest_file, 17, 20)
  ...
  if (!file.exists(local_manifest) | !file.info(local_manifest)$size) {
    ...
    nacch_dir <- paste0("/Volumes/Coastal_Change_Hazards/Archive/Data/2018/2018-302-DD_20180413_20191231/sandkey/", year, "/c1/", doy, "_", month, ".", day, "/")
      ...
      if (!file.exists(local_file) | !file.info(local_file)$size) {
        ...
        #} else {
          ...
          if (inherits(err, "try-error")) {
            ...
            cleanup <- paste0(cleanup, "del \"C:\\Users\\camerauser\\sandkey\\toBeSent\\c1\\", doy, "_", month, ".", day, "\\", hour_s, "\\", file, "\"\n")
